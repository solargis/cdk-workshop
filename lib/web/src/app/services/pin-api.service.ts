...
export class PinApiService {
  ...
  savePin(pin: Pin, unsavedImage: Image, customName: string): Observable<SavedPin> {
    return this.http
      .post<SavedPin>(this.pinApiUrl, { ...pin, customName, unsavedImage });
  }

  renamePin(pointUrl: string, customName: string) {
    return this.http.patch<SavedPin>(`${this.pinApiUrl}/${pointUrl}`, { customName });
  }
