...
export class MapComponent implements OnInit, OnChanges, OnDestroy {
    ...
    this.subscriptions.push(
      this.pins$
        .pipe(
          map(pins =>
            pins.reduce(
              (set, pin) => (set.add(pin.pointUrl), set),
              new Set<string>()
            )
          ),
          ...
          simpleChange(),
          map((pointUrlsChange: SimpleChange) =>
            diffArrays<string>(
        ...
        .subscribe(([{ added, removed }, pins]) => {
          ...
          // handle added
          const addedPins = added.map(pointUrl =>
            pins.find(pin => pin.pointUrl === pointUrl)
          )
  ...
  private createPinMarker (pin: SavedPin): L.Marker {
    const pinMarker = L.marker(pin.point, {
      draggable: false,
      zIndexOffset: 100
    })
    const icon = this.createPinMarkerIcon(pin.pointUrl)
    pinMarker.setIcon(
      new CustomIcon({ nativeElement: icon.location.nativeElement })
    )
    return pinMarker
  }

  public createPinMarkerIcon (
    pointUrl: string
  ): ComponentRef<PinMarkerComponent> {
    ...
    const pinMarkerIcon: ComponentRef<PinMarkerComponent> = compFactory.create(
      injector
    )
