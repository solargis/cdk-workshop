...
import { DeletePin, SavePin, UnselectPin, PinState, RenamePin } from '../state/pin.state';
  ...
  template: `
    <mat-card>
      <mat-card-title>
        ...
        <button *ngIf="!(info$ | async)" class="close" mat-icon-button (click)="unselectPin()">
      ...
      <div style="display:flex">
        <button mat-raised-button *ngIf="!unsavedImage && !pin.image && !(info$ | async)" color="primary" (click)="selectImage()">
          {{ 'PIN_IMAGE' | translate }}
        </button>
        <mat-progress-bar *ngIf="unsavedImage" mode="indeterminate" color="accent"></mat-progress-bar>
        <a mat-button *ngIf="pin.image?.url && !(info$ | async)" color="primary" [href]="pin.image.url" target="_blank">
          {{ 'DOWNLOAD_IMAGE' | translate }}
        </a>
        <button mat-button *ngIf="!unsavedImage && pin.pointUrl && !(info$ | async)" color="warn" (click)="deletePin()">
          {{ 'DELETE_PIN' | translate }}
        </button>
        <button mat-button *ngIf="pin && !(info$ | async)" color="primary" (click)="share()">
          {{ 'SHARE' | translate }}
        </button>
      </div>
      <div class="image">
        ...
        <app-image *ngIf="pin.image && !(info$ | async)" [image]="pin.image"></app-image>
      </div>
    </mat-card>
  `
})
